...
function t3() {
    ...
    a3.forEach(item => typeof item === 'number' ? a3_res.push(item) : item);
...
function t4() {
    ...
    let task4 = document.getElementsByClassName('task-4');

    for (const span of task4) {
        a4_res.push(span.getAttribute('data'));
    }
...
function t10() {
    let str = JSON.stringify(a10);
    a10_res = '?' + str.split(',').join('&').replace(/[{,},",]/gi, '').replaceAll(':', '=');
